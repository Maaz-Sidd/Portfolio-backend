...
router.post("/update-intro", async (req, res) => {
    try {
        ...
        const intro = await Intro.findOneAndUpdate({ _id: _id }, updateData, { new: true });
        if (!intro) {
...
router.post("/update-project", async (req, res) => {
    try {
        const { _id, ...updateData } = req.body;
        const project = await Project.findOneAndUpdate({ _id: _id }, updateData, { new: true });
        if (!project) {
...
router.post("/update-contact", async (req, res) => {
    try {
        const { _id, ...updateData } = req.body;
        const contact = await Contact.findOneAndUpdate({ _id: _id }, updateData, { new: true });
        if (!contact) {
...
export { router };
//# sourceMappingURL=portfolio.route.mjs.map